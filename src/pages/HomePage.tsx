...
export default function HomePage() {
  ...
  // Aba ativa via query string (?app=...) — sobrevive ao voltar do player.
  const appParam = searchParams.get('app')
  const activeApp = apps.find((a) => a.id === appParam) ?? apps[0]
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="anim-rise space-y-3">
        <p className="label-mono">Treinamento · sistemas da operação</p>
        <h1 className="text-4xl font-bold tracking-tight text-balance">
          O que você quer <span className="text-teal">treinar</span> hoje?
        </h1>
        <p className="max-w-2xl text-fog">
          Escolha a aplicação e o guia. No modo <strong className="text-snow">Guia</strong>, o ponto
          de clique aparece marcado na tela; no modo <strong className="text-snow">Prática</strong>,
          você recebe só a dica e precisa acertar onde clicar.
        </p>
      </section>

      {/* Abas de aplicação */}
      <div role="tablist" aria-label="Aplicações" className="glass flex w-fit gap-1 p-1.5">
        {apps.map((app) => {
          ...
          return (
            <button
              ...
              onClick={() => setSearchParams(app.id === apps[0].id ? {} : { app: app.id })}
              className={`min-h-12 rounded-lg px-6 text-base font-semibold transition active:scale-[0.97] focus-visible:outline-2 focus-visible:outline-teal ${
                active ? 'bg-teal text-teal-deep' : 'text-fog hover:text-snow'
              }`}
            >
              {app.name}
              <span
                className={`ml-2 font-mono text-xs ${active ? 'text-teal-deep/70' : 'text-fog/70'}`}
              >
                {app.guides.length}
              </span>
            </button>
      ...
      <section
        ...
        aria-label={`Guias de ${activeApp.name}`}
      >
        {activeApp.guides.map((guide, i) => (
          <article
            key={guide.id}
